---
title: JSON Calendar Spec
layout: page.vto
---

# JSON Calendar Spec

Version: `1.0`

Date: `2025-03-10`

## Spec

Version `1.0` of JSON Calendar focuses on events and core calendaring functionality such as:

- `calendar` object
- `event` object
  - Basic event information
  - Date and timezone information
  - `recurrence` rules
  - `notifications` information
  - `attendees` information
- Optional `custom` object to define extensions

## JSON Schema

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "title": "JSON Calendar File Format Schema",
  "description": "A JSON Schema for the modern .jsoncalendar file format proposal, focusing on events and core calendaring features.",
  "type": "object",
  "required": ["version", "prodID", "calendar"],
  "properties": {
    "version": {
      "type": "string",
      "enum": ["1.0"],
      "description": "The version of the jCal format."
    },
    "prodID": {
      "type": "string",
      "description": "A product identifier for the application or system generating the file."
    },
    "calendar": {
      "type": "object",
      "required": ["timezone", "events"],
      "properties": {
        "timezone": {
          "type": "string",
          "description": "An IANA time zone identifier (e.g., 'America/New_York')."
        },
        "name": {
          "type": "string",
          "description": "An optional human-readable name for the calendar."
        },
        "description": {
          "type": "string",
          "description": "An optional description for the calendar."
        },
        "events": {
          "type": "array",
          "description": "A list of calendar events.",
          "items": {
            "type": "object",
            "required": ["uid", "summary", "start", "end"],
            "properties": {
              "uid": {
                "type": "string",
                "description": "A globally unique identifier for the event."
              },
              "summary": {
                "type": "string",
                "description": "A short title or summary for the event."
              },
              "description": {
                "type": "string",
                "description": "A detailed description of the event. (UTF-8 encoded; no legacy escaping or folding needed.)"
              },
              "location": {
                "type": "string",
                "description": "The location where the event takes place."
              },
              "start": {
                "type": "string",
                "format": "date-time",
                "description": "The event start date and time in ISO 8601 format."
              },
              "end": {
                "type": "string",
                "format": "date-time",
                "description": "The event end date and time in ISO 8601 format."
              },
              "recurrence": {
                "type": "object",
                "description": "Details for recurring events.",
                "properties": {
                  "frequency": {
                    "type": "string",
                    "enum": ["DAILY", "WEEKLY", "MONTHLY", "YEARLY"],
                    "description": "The frequency of recurrence."
                  },
                  "interval": {
                    "type": "integer",
                    "minimum": 1,
                    "description": "The interval between recurrences."
                  },
                  "until": {
                    "type": "string",
                    "format": "date-time",
                    "description": "A date-time indicating when the recurrence ends."
                  },
                  "count": {
                    "type": "integer",
                    "minimum": 1,
                    "description": "The number of occurrences."
                  }
                },
                "additionalProperties": false
              },
              "notifications": {
                "type": "array",
                "description": "Optional notifications or reminders for the event.",
                "items": {
                  "type": "object",
                  "required": ["action", "trigger"],
                  "properties": {
                    "action": {
                      "type": "string",
                      "enum": ["display", "email"],
                      "description": "The type of notification."
                    },
                    "trigger": {
                      "type": "string",
                      "description": "A duration (ISO 8601) or a specific date-time when the notification should trigger."
                    },
                    "message": {
                      "type": "string",
                      "description": "The message to be displayed or sent with the notification."
                    }
                  },
                  "additionalProperties": false
                }
              },
              "attendees": {
                "type": "array",
                "description": "Optional list of event attendees.",
                "items": {
                  "type": "object",
                  "required": ["name", "email"],
                  "properties": {
                    "name": {
                      "type": "string",
                      "description": "The attendee's name."
                    },
                    "email": {
                      "type": "string",
                      "format": "email",
                      "description": "The attendee's email address."
                    },
                    "responseStatus": {
                      "type": "string",
                      "enum": ["accepted", "declined", "tentative", "needs-action"],
                      "description": "The attendee's response status."
                    }
                  },
                  "additionalProperties": false
                }
              }
            },
            "additionalProperties": false
          }
        },
        "custom": {
          "type": "object",
          "description": "An optional object for custom extensions.",
          "additionalProperties": true
        }
      },
      "additionalProperties": false
    }
  },
  "additionalProperties": false
}
```
